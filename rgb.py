        ...
        if len(merged_lines) == 0:
            ...
            for i, merged_line in enumerate(merged_lines):
                ...
                if abs(x1 - x3) < threshold and abs(y1 - y3) < threshold and abs(x2 - x4) < threshold and abs(y2 - y4) < threshold:
                    merged_lines[i] = (min(x1, x3), min(y1, y3), max(x2, x4), max(y2, y4))
                    merged = True
                    break
            if not merged:
                merged_lines.append(line)
...
# 读取图像
image = cv2.imread('example/4.jpg')
...
lower_color = np.array([250, 160, 0])  # 根据具体情况调整
upper_color = np.array([255, 180, 10])  # 根据具体情况调整
...
# Canny边缘检测
edges = cv2.Canny(erosion, 200, 350)
...
if lines is not None:
    ...
    for i in range(len(sorted_lines) - 1):
        for j in range(i + 1, len(sorted_lines)):
            if abs(np.arctan2(sorted_lines[i][3] - sorted_lines[i][1], sorted_lines[i][2] - sorted_lines[i][0]) - np.arctan2(sorted_lines[j][3] - sorted_lines[j][1], sorted_lines[j][2] - sorted_lines[j][0])) < 0.1:
                if np.sqrt((sorted_lines[i][2] - sorted_lines[i][0]) ** 2 + (sorted_lines[i][3] - sorted_lines[i][1]) ** 2) > np.sqrt((sorted_lines[j][2] - sorted_lines[j][0]) ** 2 + (sorted_lines[j][3] - sorted_lines[j][1]) ** 2):
                    sorted_lines[j] = sorted_lines[i]
                else:
    ...
    # 获取四个顶点
    # 合并线段
    final_lines = merge_lines(final_lines, 4)
